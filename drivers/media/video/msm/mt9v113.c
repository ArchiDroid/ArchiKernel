...
static int prev_brightness_mode;
 /*LGE_CHANGE_S : Add night mode
  2012-02-02, suk.kitak@lge.com, */
static int prev_scene_mode;
/* LGE_CHANGE_E :  */


/* LGE_CHANGE_S :  Add for preview fps range 2012.01.12, suk.kitak@lge.com  */
#if 1 // donghyun.kwon(20111019) : build_error_fixed
/* LGE_CHANGE_E*/
...
static int mt9v113_polling_reg(unsigned short waddr, unsigned short wcondition, unsigned short result, int delay, int time_out)
{
	...
	for(i=0; i<time_out; i++){
		...
		if (rc<0) {
			printk("polling reg failed to read\n");
			return rc;
		}
...
int mt9v113_check_sensor_mode(void)
{
	...
	for(i=0; i<50; i++){
		...
		if (rc < 0){
			printk("mt9v113: reading mcu_address_sts fail\n");
			return rc;
		}
		...
		if (rc < 0){
			printk("mt9v113: reading mcu_data fail\n");
			return rc;
		}
...
static long mt9v113_reg_init(void)
{
	int32_t rc = 0;
		...
		if (!pstRegisterList)
		{
			printk("[miracle.kim] param file open failed");
	...
	if (rc < 0)
        ...
        rc = mt9v113_i2c_write_table(mt9v113_regs.refreshmode, mt9v113_regs.refreshmode_size);
        if (rc < 0)
            return rc;    

        rc = mt9v113_check_sensor_mode();

        rc = mt9v113_i2c_write_table(mt9v113_regs.refresh, mt9v113_regs.refresh_size);
        if (rc < 0)
            return rc;    
    ...
    if (rc<0)
    {
        printk("###[ERROR]%s: failed to check sensor mode\n", __func__);
        return rc;
	}
...
#if 1
/* LGE_CHANGE_S : Camera setting interface
 * 2011-11-19, donghyun.kwon@lge.com, 
 * Add exposure compensation for front camera
 */
 /*LGE_CHANGE_S : Add night mode
  2012-02-02, suk.kitak@lge.com, */
...
//static int brightness_table[] = {0x0015, 0x0019, 0x001D, 0x0021, 0x0025, 0x0029, 0x002C, 0x0030, 0x0033, 0x0037, 0x003A, 0x003E, 0x0042, 0x0046, 0x004A, 0x004E, 0x0052, 0x0056, 0x005A, 0x005E, 0x0062, 0x0066, 0x006A, 0x006E, 0x0072};
/* LGE_CHANGE_E :  */
/* LGE_CHANGE_E : Camera setting interface */
#else
...
static int mt9v113_set_brightness(int mode)
{
	 ...
	 * Add exposure compensation for front camera
	 */
       int brghtness_max_level = (sizeof(brightness_table) / sizeof(brightness_table[0]))-1;
	/* LGE_CHANGE_E : Camera setting interface */
	 ...
	 * Add exposure compensation for front camera
	 */
	if(mode < 0 || mode > brghtness_max_level){
	/* LGE_CHANGE_E : Camera setting interface */
#else
	if(mode < 0 || mode > 12){
#endif
		printk("###[ERROR]%s: Invalid Mode value\n", __func__);
		return -EINVAL;	
	}
...
static int mt9v113_set_contrast (int contrast)
{
    ...
    printk(KERN_ERR "[suk.kitak] mt9v113_set_contrast!!!!!!%d \n",contrast);
    if(prev_scene_mode == contrast)
    {   
        return rc;
    }
    ...
    {
        case 0:     //normal mode
	 	rc = mt9v113_i2c_write_table(mt9v113_regs.scene_normal_mode_tbl, mt9v113_regs.scene_normal_mode_tbl_size);
		if (rc < 0)
			return rc;
            break;

        case 6:     //night mode
	 	rc = mt9v113_i2c_write_table(mt9v113_regs.scene_night_mode_tbl, mt9v113_regs.scene_night_mode_tbl_size);
		if (rc < 0)
			return rc;
            break;
...
static long mt9v113_set_sensor_mode(int mode)
{
	...
	if (config_csi == 0) {
		...
		if (rc < 0)
			printk("config csi controller failed \n");
	...
	case SENSOR_PREVIEW_MODE:
		for (retry = 0; retry < 3; ++retry) {
			...
			if (rc < 0)
				printk("[ERROR]%s:Sensor Preview Mode Fail\n", __func__);
			else {
	...
	case SENSOR_RAW_SNAPSHOT_MODE:
		for (retry = 0; retry < 3; ++retry) {
			...
			if (rc < 0)
				printk("[ERROR]%s:Sensor Snapshot Mode Fail\n", __func__);
			else {
...
#if 1 /* LGE_CHANGE_S :  Add for preview fps range 2012.01.12, suk.kitak@lge.com  */
// LGE_CAMERA_S : Adjust VT Cam frame rate - jonghwan.ko@lge.com
static int mt9v113_set_Fps(int fps)
{
	int32_t rc = 0;
...
/*LGE_CHANGE_S : Add night mode
  2012-02-02, suk.kitak@lge.com, */
		case CFG_SET_CONTRAST:
			rc = mt9v113_set_contrast(cfg_data.mode);
			break;
/* LGE_CHANGE_E :  */
 ...
 // LGE_CAMERA_E : Adjust VT Cam frame rate - jonghwan.ko@lge.com			 
#endif
/* LGE_CHANGE_S :  Add for preview fps range 2011.12.02, samjinjang@lge.com  */
		case CFG_SET_FPS :
			rc = mt9v113_set_Fps(cfg_data.mode);
			break;
/* LGE_CHANGE_E */
...
static int mt9v113_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
	...
	if(rc<0){
		printk(" mt9v113_sensor_init_probe fails\n");
		return rc;
	}
...
#endif
	/* LGE_CHANGE_S : VGA Camera rcommand
	 * 2011-12-14, donghyun.kwon@lge.com, 
	 * MT9V113 needs a delay at least 2mm
	 */
	mdelay(2);	
	/* LGE_CHANGE_E : VGA Camera rcommand */
...
//	msm_camio_camif_pad_reg_reset();
	...
	if (rc < 0) {
		printk("mt9v113_sensor_init failed!\n");
		goto init_fail;
	}
				...
				struct msm_sensor_ctrl *s)
{
	...
	s->s_camera_type = FRONT_CAMERA_2D;
       /* LGE_CHANGE_S : VGA Camera mount angle
        * 2011-12-10, donghyun.kwon@lge.com, 
        * Camera sensor's mount angle value is set from the board info
        */
	s->s_mount_angle  = info->sensor_platform_info->mount_angle;
       /* LGE_CHANGE_E : VGA Camera mount angle */
probe_done:
