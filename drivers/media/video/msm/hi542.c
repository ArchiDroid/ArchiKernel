...
#define HI542_5M_Bayer_QTR_SIZE_BLANKING_PIXELS  	 1503 // for flicker 1502 // 1496 // in pixels
#define HI542_5M_Bayer_QTR_SIZE_BLANKING_LINES           35 // for flicker  15   // in lines
...
#define HI542_5M_Bayer_FULL_SIZE_BLANKING_PIXELS	 215 // for flicker 214//182
#define HI542_5M_Bayer_FULL_SIZE_BLANKING_LINES          59 // for flicker  39//15  // in lines
	...
	unsigned short raddr, unsigned short *rdata)
{
	...
	if (rc < 0) {
		pr_err("hi542_i2c_read() hi542_i2c_rxdata failed!\n");
		return rc;
	}
...
//	 int16_t target_dist, small_step, next_lens_position;		 //Remove this part if you use slew mode.
	...
	if(step_direction < 0) {
		if(num_steps > 20) {
			/*macro to infinity*/
			hw_damping = 0xF;	//0x5;	/* LGE_CHANGED donghyun.kwon(20120317) : add workaround rutines for dampper */
			...
			hi542_sw_damping_time_wait = 4500;
		} else {
			/*reverse search Coarse Jump ( > 4) dir - macro to infinity*/
			hw_damping = 0xF;	//0x5;//0x8;	/* LGE_CHANGED donghyun.kwon(20120317) : add workaround rutines for dampper */
			hi542_sw_damping_time_wait = 4500;
			}
	} else {
		if(num_steps > 4) {
	 ...
	 if(hi542_ctrl->curr_lens_pos != dest_lens_position){
		...
		code_val_lsb = (dest_lens_position & 0x00FF);
		 ...
		 if (rc < 0){
			pr_err("hi542_move_focus failed writing i2c at line %d ...\n", __LINE__);
			 return rc;
}
...
/*LGE_CHANGE_S : Hi542 sensor stabilization
  ...
  apply X-shutdown mode,sensor reset stabilization*/
	...
	hi542_i2c_write_b_sensor(0x03d4, 0x18);/* LDO level */
	msleep(3);
/* LGE_CHANGE_E : Hi542 sensor stabilization */	
...
int hi542_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	...
	pr_err("[agkwon 2012 0415 2048] hi542_sensor_open_init");
