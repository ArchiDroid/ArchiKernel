...
int	load_command( struct command_info *store_command, struct load_command_data *load_data, unsigned int count )
{
	...
	for( loop = 0 ; loop < count ; ++loop )
	{
		...
		store_command_data->function = command->function, store_command_data->parameter_string = command->parameter_string, store_command_data->parameter_count = command->parameter_count;
