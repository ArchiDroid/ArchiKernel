		...
		struct device_attribute *attr, char *buf)
{
	...
	if (data_length) {
		retval = rmidev->fn_ptr->read(rmidev->rmi4_data,
				...
				data_length);
		if (retval < 0) {
			printk( "ITUCH : Device(%s), Failed to read data\n", dev_name( &rmidev->rmi4_data->i2c_client->dev ) );
			return retval;
		}
	} else {
		return -EINVAL;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (data_length) {
		retval = rmidev->fn_ptr->write(rmidev->rmi4_data,
				...
				data_length);
		if (retval < 0) {
			printk( "ITUCH : Device(%s), Failed to write data\n", dev_name( &rmidev->rmi4_data->i2c_client->dev ) );
			return retval;
		}
	} else {
		return -EINVAL;
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		printk( "ITUCH : Device(%s), New position 0x%04x is invalid\n", dev_name( &rmidev->rmi4_data->i2c_client->dev ), ( unsigned int )newpos );
		newpos = -EINVAL;
		goto clean_up;
	}
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!rmidev) {
		printk( "ITUCH : Device(%s), Failed to alloc mem for rmidev\n", dev_name( &rmi4_data->i2c_client->dev ) );
		retval = -ENOMEM;
		goto err_rmidev;
	}
	...
	if (!rmidev) {
		printk( "ITUCH : Device(%s), Failed to alloc mem for fn_ptr\n", dev_name( &rmi4_data->i2c_client->dev ) );
		retval = -ENOMEM;
		goto err_fn_ptr;
	}
	...
	if (retval < 0) {
		printk( "ITUCH : Device(%s), Failed to create device class\n", dev_name( &rmi4_data->i2c_client->dev ) );
	...
	if (rmidev_major_num) {
		...
		if (retval < 0) {
			printk( "ITUCH : Device(%s), Failed to allocate char device region\n", dev_name( &rmi4_data->i2c_client->dev ) );
	...
	if (!dev_data) {
		printk( "ITUCH : Device(%s), Failed to alloc mem for dev_data\n", dev_name( &rmi4_data->i2c_client->dev ) );
		retval = -ENOMEM;
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		printk( "ITUCH : Device(%s), Failed to add rmi char device\n", dev_name( &rmi4_data->i2c_client->dev ) );
	...
	if (IS_ERR(device_ptr)) {
		printk( "ITUCH : Device(%s), Failed to create rmi char device\n", dev_name( &rmi4_data->i2c_client->dev ) );
		retval = -ENODEV;
		goto err_char_device;
	}
	...
	if (retval < 0) {
		printk( "ITUCH : Device(%s), Failed to export attention gpio\n", dev_name( &rmi4_data->i2c_client->dev ) );
	} else {
		retval = gpio_export_link(&(rmi4_data->input_dev->dev),
				"attn", rmi4_data->board->gpio);
		if (retval < 0) {
			printk( "ITUCH : Device(%s), Failed to create gpio symlink\n", dev_name( &rmi4_data->input_dev->dev ) );
		} else {
	...
	if (!rmidev->sysfs_dir) {
		printk( "ITUCH : Device(%s), Failed to create sysfs directory\n", dev_name( &rmi4_data->i2c_client->dev ) );
		goto err_sysfs_dir;
	}
