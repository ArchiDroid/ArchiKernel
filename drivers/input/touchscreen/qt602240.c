...
#include <linux/slab.h>     //added by hongz
...
static report_id_map_t *report_id_map=(report_id_map_t *)&report_id_map_data;
...
void qt_Acquisition_Config_Init(void)
{
    ...
    acquisition_config.tchdrift = 5;//20; //5; // 4s
    acquisition_config.driftst = 5;//20; //0; // 4s

    acquisition_config.tchautocal = 0; // infinite
    acquisition_config.sync = 0; // disabled

    acquisition_config.atchcalst = 9;
    acquisition_config.atchcalsthr = 23;//35;	// 35;
...
void qt_Multitouchscreen_Init(void)
{
    ...
    touchscreen_config.orient = 1;//0;//7;//3;
    ...
    touchscreen_config.xedgedist = 70; //40;
    touchscreen_config.yedgectrl = 136; //143;
    touchscreen_config.yedgedist = 70; //80;
...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	noise_suppression_config.noisethr = 0; //30; 	//35;

	noise_suppression_config.freqhopscale = 0;///1;

    noise_suppression_config.freq[0] = 10;//5; //29; 	//5;	// 6;//10;
    noise_suppression_config.freq[1] = 15;//10; //34;	//15;	// 11;//15;
    noise_suppression_config.freq[2] = 20;//15; //39;	//25;	//16;//20;
    noise_suppression_config.freq[3] = 25;//20; //49;	//35;	// 19;//25;
    noise_suppression_config.freq[4] = 30;//25; //58;	//45;	// 21;//30;
...
void qt_CTE_Config_Init(void)
{
    ...
    cte_config.idlegcafdepth = 8;//16;///4;
    cte_config.actvgcafdepth = 16; //63;	//8;

    cte_config.voltage = 10;//15; //0x3c;
...
//			QT_printf("[TSP] Report ID : %d, Object Type : T%d, Instance : %d\n\r",report_id ,object_type,instance);
        ...
        QT_printf("[TSP] Chip setup sequence is now configuring...!\n\r");
		       ...
		       const struct i2c_device_id *id)
{
	...
	qt602240_power_onoff(1); /* bongkyu.kim test */
...
int __init qt602240_init(void)
{
	...
	qt602240 = (struct qt602240_data *)kzalloc(sizeof(struct qt602240_data), GFP_KERNEL);
	if (qt602240 == NULL) {
		return -ENOMEM;
	}
