...
int rmi_register_phys_device(struct rmi_phys_device *phys)
{
	...
	if (!pdata) {
		printk( "BTUCH : device(%s), no platform data!!!\n", dev_name( phys->dev ) );
		return -EINVAL;
	}
...
int rmi_register_function_driver(struct rmi_function_handler *fh)
{
	...
	if (fh_dup) {
		printk( "BTUCH : function f%.2x already registered!!!\n", fh->func );
		return -EINVAL;
	}
...
static int rmi_register_character_device(struct device *dev, void *data)
{
	...
	if (!char_driver->init) {
		printk( "BTUCH : ERROR!!! device(%s) No init() function\n", dev_name( dev ) );
		return -EINVAL;
	}
	...
	if (!retval) {
		printk( "BTUCH : Failed to register character device(%s)\n", dev_name( dev ) );
		goto error_exit;
	}

	retval = char_driver->init(char_dev);
	if (retval) {
		printk( "BTUCH : Failed to initialize character device(%s)\n", dev_name( dev ) );
		goto error_exit;
	}
	...
	printk( "BTUCH : Registered a device(%s)\n", dev_name( &char_dev->dev ) );
	return retval;
...
int rmi_register_character_driver(struct rmi_char_driver *char_driver)
{
	...
	if (retval) {
		printk( "BTUCH : Failed to register %s(%d)\n", char_driver->driver.name, retval );
		return retval;
	}
...
void rmi_get_bus_data(int (*fn)(struct device*, void*), void *data)
{
	bus_for_each_dev(&rmi_bus_type, NULL, data, *fn);
}
EXPORT_SYMBOL(rmi_get_bus_data);
...
static int __init rmi_bus_init(void)
{
	...
	if (error < 0) {
		printk( "BTUCH : error registering the RMI bus(%d)\n", error );
		return error;
	}
