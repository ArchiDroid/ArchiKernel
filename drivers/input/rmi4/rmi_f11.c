...
static void rmi_f11_finger_handler(struct f11_2d_sensor *sensor)
{
	...
	for (i = 0, finger_pressed_count = 0; i < sensor->nbr_fingers; i++) {
		...
		if (finger_state == F11_RESERVED) {
			printk( "FTUCH : F11 invalid finger state[%d]:0x%02x\n", i, finger_state );
			continue;
...
static int f11_read_control_regs(struct rmi_device *rmi_dev,
	...
	error = rmi_read_block(rmi_dev, read_address, ctrl->ctrl0_9,
		sizeof(ctrl->ctrl0_9));
	if (error < 0) {
		printk( "FTUCH : F11 Device(%s) failed to read ctrl0_9(%d)\n", dev_name( &rmi_dev->dev ), error );
		return error;
	}
	...
	if (ctrl->ctrl10) {
		...
		if (error < 0) {
			printk( "FTUCH : F11 Device(%s) failed to read ctrl0(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (ctrl->ctrl11) {
		...
		if (error < 0) {
			printk( "FTUCH : F11 Device(%s) failed to read ctrl11(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (ctrl->ctrl14) {
		...
		if (error < 0) {
			printk( "FTUCH : F11 Device(%s) failed to read ctrl14(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (ctrl->ctrl15) {
		...
		if (error < 0) {
			printk( "FTUCH : F11 Device(%s) failed to read ctrl15(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (ctrl->ctrl16) {
		...
		if (error < 0) {
			printk( "FTUCH : F11 Device(%s) failed to read ctrl16(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (ctrl->ctrl17) {
		...
		if (error < 0) {
			printk( "FTUCH : F11 Device(%s) failed to read ctrl17(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (ctrl->ctrl18_19) {
		...
		if (error < 0) {
			printk( "FTUCH : F11 Device(%s) failed to read ctrl18_19(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (ctrl->ctrl20_21) {
		...
		if (error < 0) {
			printk( "FTUCH : F11 Device(%s) failed to read ctrl20_21(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
					...
					int ctrl_base_addr)
{
	...
	if (ctrl->ctrl12 && ctrl->ctrl12_size && query->configurable) {
		if (ctrl->ctrl12_size > query->max_electrodes) {
			printk( "FTUCH : F11 Device(%s) invalid cfg size(%d), should be < %d\n", dev_name( &rmi_dev->dev ), ctrl->ctrl12_size, query->max_electrodes );
			return -EINVAL;
		}
		error = rmi_write_block(rmi_dev, write_address,
						&ctrl->ctrl12->reg,
...
static void f11_set_abs_params(struct rmi_function_container *fc, int index)
{
	...
	printk( "FTUCH : F11 Device(%s) resolution min(%dX%d), max(%dX%d)\n", dev_name( &fc->dev ), x_min, y_min, x_max, y_max );
	...
	*/
	...
	for (i = 0; i < (f11->dev_query.nbr_of_sensors + 1); i++) {
		...
		if (f11->dev_query.has_query9) {
			...
			if (rc < 0) {
				printk( "FTUCH : F11 Device(%s) failed to read query 9\n", dev_name( &fc->dev ) );
				return rc;
			}
			query_offset += rc;
		}
		...
		if (rc < 0) {
			printk( "FTUCH : F11 Device(%s) failed to initialize control params\n", dev_name( &fc->dev ) );
			return rc;
		}
	...
	for (i = 0; i < (f11->dev_query.nbr_of_sensors + 1); i++) {
		...
		* phys name for input device */
		...
		if (f11->sensors[i].sens_query.has_gestures) {
			...
			if (!vm_pdata) {
				printk( "FTUCH : F11 Device(%s) failed to get the pdata virtualbutton map\n", dev_name( &fc->dev ) );
				goto error_unregister;
			}
			...
			vm_sensor->map = kcalloc(vm_pdata->buttons,
					...
					GFP_KERNEL);
			if (!vm_sensor->map) {
				printk( "FTUCH : F11 Device(%s) failed to allocate the virtualbutton map\n", dev_name( &fc->dev ) );
				rc = -ENOMEM;
				goto error_unregister;
			}
...
static int rmi_f11_create_sysfs(struct rmi_function_container *fc)
{
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		if (sysfs_create_file
			...
			printk( "FTUCH : F11 Device(%s) failed to create sysfs file for %s\n", dev_name( &fc->dev ), attrs[attr_count].attr.name );
			rc = -ENODEV;
...
static int rmi_f11_resume(struct rmi_function_container *fc)
{
	...
	if (retval < 0) {
		printk( "FTUCH : F11 Device(%s) failed to issue rezero command(%d)\n", dev_name( &rmi_dev->dev ), retval );
		return retval;
	}
...
static int __init rmi_f11_module_init(void)
{
	...
	if (error < 0) {
		printk( "FTUCH : F11 register failed!!!\n" );
		return error;
	}
				     ...
				     const char *buf, size_t count)
{
	...
	if (newValue < 0 || newValue > 1) {
		printk( "FTUCH : F11 Device(%s) rezeroOnResume must be either 1 or 0\n", dev_name( dev ) );
		return -EINVAL;
	}
				     ...
				     const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &newValue) != 1)
		return -EINVAL;
	if (newValue < 0) {
		printk( "FTUCH : F11 Device(%s) rezeroWait must be 0 or greater\n", dev_name( dev ) );
		return -EINVAL;
	}
