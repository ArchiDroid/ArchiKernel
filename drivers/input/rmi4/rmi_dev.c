...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (IS_ERR(data)) {
		printk( "DTUCH : pointer of char device is invalid\n" );
		return -EBADF;
	}
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		printk( "DTUCH : device(%s) newpos 0x%04x is invalid\n", dev_name( &data->rmi_dev->dev ), ( unsigned int )newpos );
		newpos = -EINVAL;
		goto clean_up;
	}
...
static int __init rmidev_init(void)
{
	...
	if (IS_ERR(rmidev_device_class)) {
		printk( "DTUCH : ERROR - Failed to create /dev/%s\n", CHAR_DEVICE_NAME );
		return -ENODEV;
	}
	/* setup permission */
