...
static ssize_t delay_write(struct file *filp, const char __user *buffer,
	...
	if (retval != 5) {
		printk( "STUCH : Device(%s) incorrect number of values provided for delay\n", dev_name( &data->rmi_dev->dev ) );
		return -EINVAL;
	}
	if (new_read_delay < 0) {
		printk( "STUCH : Device(%s) byte delay must be positive microseconds\n", dev_name( &data->rmi_dev->dev ) );
		return -EINVAL;
	}
	if (new_write_delay < 0) {
		printk( "STUCH : Device(%s) write delay must be positive microseconds\n", dev_name( &data->rmi_dev->dev ) );
		return -EINVAL;
	}
	if (new_block_delay < 0) {
		printk( "STUCH : Device(%s) block delay must be positive microseconds\n", dev_name( &data->rmi_dev->dev ) );
		return -EINVAL;
	}
	if (new_pre_delay < 0) {
		printk( "STUCH : Device(%s) pre-transfer delay must be positive microseconds\n", dev_name( &data->rmi_dev->dev ) );
		return -EINVAL;
	}
	if (new_post_delay < 0) {
		printk( "STUCH : Device(%s) post-transfer delay must be positive microseconds\n", dev_name( &data->rmi_dev->dev ) );
		return -EINVAL;
	}
...
static int setup_debugfs(struct rmi_device *rmi_dev) {
	...
	if (!data->debugfs_phys || IS_ERR(data->debugfs_phys)) {
		printk( "STUCH : Device(%s) failed to create debugfs phys\n", dev_name( &rmi_dev->dev ) );
...
static void rmi_free_function_list(struct rmi_device *rmi_dev)
{
	...
	if (!data) {
		printk( "STUCH : Device(%s) No driver data\n", dev_name( &rmi_dev->dev ) );
		return;
	}
			     ...
			     struct rmi_function_container *fc)
{
	...
	if (retval) {
		printk( "STUCH : Device(%s) failed device_register for F%02X\n", dev_name( &rmi_dev->dev ), fc->fd.function_number );
		return retval;
	}

	retval = fc->fh->init(fc);
	if (retval < 0) {
		printk( "STUCH : Device(%s) failed to initialize function F%02x\n", dev_name( &rmi_dev->dev ), fc->fd.function_number );
		goto error_exit;
	}
...
static int rmi_driver_process_reset_requests(struct rmi_device *rmi_dev)
{
	...
	if (data->f01_container && data->f01_container->fh &&
		...
		if (retval < 0) {
			printk( "STUCH : Device(%s) F%02x reset handler failed(%d)\n", dev_name( dev ), data->f01_container->fh->func, retval );
			return retval;
		}
	}
	...
	list_for_each_entry(entry, &data->rmi_functions.list, list) {
		if (entry->fh && entry->fh->reset) {
			...
			if (retval < 0) {
				printk( "STUCH : Device(%s) F%02x reset handler failed(%d)\n", dev_name( dev ), entry->fh->func, retval );
				return retval;
			}
		}
	}
...
static int rmi_driver_process_config_requests(struct rmi_device *rmi_dev)
{
	...
	if (data->f01_container && data->f01_container->fh &&
		...
		if (retval < 0) {
			printk( "STUCH : Device(%s) F%02x config handler failed(%d)\n", dev_name( dev ), data->f01_container->fh->func, retval );
			return retval;
		}
	}
...
static int process_interrupt_requests(struct rmi_device *rmi_dev)
{
	...
	if (error < 0) {
		printk( "STUCH : Device(%s) failed to read irqs\n", dev_name( dev ) );
		return error;
	}
	/* Device control (F01) is handled before anything else. */
	...
	list_for_each_entry(entry, &data->rmi_functions.list, list)
		if (entry->irq_mask && entry->fh && entry->fh->attention) {
			...
			if (u8_is_any_set(irq_bits, data->num_of_irq_regs)) {
				...
				if (error < 0)
					printk( "STUCH : Device(%s) f%.2x attention handler failed(%d)\n", dev_name( dev ), entry->fh->func, error );
			}
		}
...
static int rmi_driver_irq_handler(struct rmi_device *rmi_dev, int irq)
{
	...
	if (!data || !data->f01_container || !data->f01_container->fh) {
		printk( "STUCH : Device(%s) not ready to handle interrupts yet!\n", dev_name( &rmi_dev->dev ) );
		return 0;
	}
...
static int rmi_driver_reset_handler(struct rmi_device *rmi_dev)
{
	 ...
	 */
	if (!data || !data->f01_container || !data->f01_container->fh) {
		printk( "STUCH : Device(%s) not ready to handle reset yet!\n", dev_name( &rmi_dev->dev ) );
		return 0;
	}
...
static int rmi_driver_irq_save(struct rmi_device *rmi_dev, u8 * new_ints)
{
	...
	if (!data->irq_stored) {
		...
		if (retval < 0) {
			printk( "STUCH : Device(%s) failed to read enabled interrupts!!!\n", dev_name( dev ) );
			goto error_unlock;
		}
		/*
		...
		if (retval < 0) {
			printk( "STUCH : Device(%s) failed to change enabled interrupts!!!\n", dev_name( dev ) );
			goto error_unlock;
		}
		...
		printk( "STUCH : Device(%s) attempted to save values when already stored!!!\n", dev_name( dev ) );
	}
...
static int rmi_driver_irq_restore(struct rmi_device *rmi_dev)
{
	...
	if (data->irq_stored) {
		retval = rmi_write_block(rmi_dev,
				...
				data->irq_mask_store, data->num_of_irq_regs);
		if (retval < 0) {
			printk( "STUCH : Device(%s) failed to write enabled interupts!!!\n", dev_name( dev ) );
			goto error_unlock;
		}
		...
		retval = -EINVAL;
		printk( "STUCH : Device(%s) attempted to restore values when not stored!!!\n", dev_name( dev ) );
	}
				     ...
				     u16 page_start)
{
	...
	printk( "STUCH : Device(%s) initializing F%02X for %s\n", dev_name( dev ), pdt_ptr->function_number, pdata->sensor_name );

	fc = kzalloc(sizeof(struct rmi_function_container),
			GFP_KERNEL);
	if (!fc) {
		printk( "STUCH : Device(%s) failed to allocate container for F%02X\n", dev_name( dev ), pdt_ptr->function_number );
		retval = -ENOMEM;
	...
	*current_irq_count += fc->num_of_irqs;
	...
	if (retval < 0) {
		printk( "STUCH : Device(%s) failed to initialize F%.2x\n", dev_name( dev ), pdt_ptr->function_number );
		goto error_free_data;
	}
				     ...
				     u16 page_start)
{
	...
	if (retval) {
		printk( "STUCH : Device(%s) failed to read device status\n", dev_name( dev ) );
		return retval;
	}

	printk( "STUCH : Device(%s) initializing F01 for %s\n", dev_name( dev ), pdata->sensor_name );
	if (!fh)
	...
	*current_irq_count += fc->num_of_irqs;
	...
	retval = device_register(&fc->dev);
	if (retval) {
		printk( "STUCH : Device(%s) failed device_register for F01\n", dev_name( dev ) );
		goto error_free_data;
	}
	...
	if (device_status.flash_prog)
		printk( "STUCH : Device(%s) RMI4 device is in bootloader mode!\n", dev_name( dev ) );
...
static int do_initial_reset(struct rmi_device *rmi_dev)
{
	...
	for (page = 0; (page <= RMI4_MAX_PAGE) && !done; page++) {
		...
		for (i = pdt_start; i >= pdt_end; i -= sizeof(pdt_entry)) {
			...
			if (retval != sizeof(pdt_entry)) {
				printk( "STUCH : Device(%s) read PDT entry at 0x%04x failed(%d)\n", dev_name( dev ), i, retval );
				return retval;
			}
			...
			if (pdt_entry.function_number == 0x01) {
				...
				retval = rmi_write_block(rmi_dev, cmd_addr,
						&cmd_buf, 1);
				if (retval < 0) {
					printk( "STUCH : Device(%s) initial reset failed(%d)\n", dev_name( dev ), retval );
					return retval;
				}
	...
	printk( "STUCH : Device(%s) failed to find F01 for initial reset\n", dev_name( dev ) );
	return -ENODEV;
}
...
static int rmi_scan_pdt(struct rmi_device *rmi_dev)
{
	...
	printk( "STUCH : Device(%s) scanning PDT\n", dev_name( dev ) );
	...
	for (page = 0; (page <= RMI4_MAX_PAGE) && !done; page++) {
		...
		for (i = pdt_start; i >= pdt_end; i -= sizeof(pdt_entry)) {
			retval = rmi_read_block(rmi_dev, i, (u8 *)&pdt_entry,
					       sizeof(pdt_entry));
			if (retval != sizeof(pdt_entry)) {
				printk( "STUCH : Device(%s) read PDT entry at 0x%04x failed\n", dev_name( dev ), i );
				goto error_exit;
			}
...
static int rmi_driver_probe(struct rmi_device *rmi_dev)
{
	...
	if (!data) {
		printk( "STUCH : Device(%s) failed to allocate driver data\n", dev_name( dev ) );
		return -ENOMEM;
	}
	...
	if (retval)
		printk( "STUCH : Device(%s) RMI initial reset failed! Soldiering on\n", dev_name( dev ) );

	retval = rmi_scan_pdt(rmi_dev);
	if (retval) {
		printk( "STUCH : Device(%s) PDT scan for %s failed(%d)\n", dev_name( dev ), pdata->sensor_name, retval );
		goto err_free_data;
	}

	if (!data->f01_container) {
		printk( "STUCH : Device(%s) missing F01 container!!!\n", dev_name( dev ) );
		retval = -EINVAL;
		goto err_free_data;
	}
	...
	if (!data->f01_container->irq_mask) {
		printk( "STUCH : Device(%s) failed to allocate F01 IRQ mask\n", dev_name( dev ) );
		retval = -ENOMEM;
		goto err_free_data;
	}
	...
	if (retval < 0) {
		printk( "STUCH : Device(%s) failed to initialize F01\n", dev_name( dev ) );
		goto err_free_data;
	}
	...
	if (retval < 0) {
		 ...
		 */
		printk( "STUCH : Device(%s) could not read PDT properties from 0x%04x, assuming 0x00\n", dev_name( dev ), PDT_PROPERTIES_LOCATION );
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			printk( "STUCH : Device(%s) failed to create sysfs file %s\n", dev_name( dev ), attrs[attr_count].attr.name );
			goto err_free_data;
		}
	}
	if (has_bsr(data)) {
		...
		if (retval < 0) {
			printk( "STUCH : Device(%s) failed to create sysfs file bsr\n", dev_name( dev ) );
			goto err_free_data;
		}
	}
	...
	if (!data->current_irq_mask) {
		printk( "STUCH : Device(%s) failed to allocate current_irq_mask\n", dev_name( dev ) );
		retval = -ENOMEM;
		goto err_free_data;
	}
	retval = rmi_read_block(rmi_dev,
	...
	if (retval < 0) {
		printk( "STUCH : Device(%s) failed to read current IRQ mask\n", dev_name( dev ) );
		goto err_free_data;
	}
	data->irq_mask_store = kzalloc(sizeof(u8) * data->num_of_irq_regs,
				       GFP_KERNEL);
	if (!data->irq_mask_store) {
		printk( "STUCH : Device(%s) failed to allocate mask store\n", dev_name( dev ) );
		retval = -ENOMEM;
		goto err_free_data;
	}
				    ...
				    const char *buf, size_t count)
{
	...
	if (retval < 0) {
		printk( "STUCH : Device(%s) invalid value '%s' written to BSR\n", dev_name( dev ), buf );
		return -EINVAL;
	}

	retval = rmi_write(rmi_dev, BSR_LOCATION, (unsigned char)val);
	if (retval) {
		printk( "STUCH : Device(%s) failed to write bsr %u to 0x%x\n", dev_name( dev ), ( unsigned int )val, BSR_LOCATION );
		return retval;
	}
					...
					const char *buf, size_t count)
{
	...
	if (new_value) {
		...
		if (retval) {
			printk( "STUCH : Device(%s) failed to enable sensor(%d)\n", dev_name( dev ), retval );
			return -EIO;
		}
	} else {
