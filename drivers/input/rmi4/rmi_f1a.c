	...
	struct f1a_data *f1a)
{
	...
	if (button_query->has_general_control) {
		...
		if (error < 0) {
			printk( "FTUCH : F1A Device(%s) failed to read f1a_0d_control_0(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (button_query->has_interrupt_enable &&
		...
		error = rmi_read_block(rmi_dev, ctrl_base_addr,
			...
			button_bitmask_size);
		if (error < 0) {
			printk( "FTUCH : F1A Device(%s) failed to read f1a_0d_control_1(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (button_query->has_multibutton_select &&
		...
		error = rmi_read_block(rmi_dev, ctrl_base_addr,
				...
				button_bitmask_size);
		if (error < 0) {
			printk( "FTUCH : F1A Device(%s) failed to read f1a_0d_control_2(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
		ctrl_base_addr = ctrl_base_addr + button_bitmask_size;
	}
	...
	if (button_query->has_tx_rx_map &&
		...
		if (error < 0) {
			printk( "FTUCH : F1A Device(%s) failed to read f1a_0d_control_3_4(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
		ctrl_base_addr = ctrl_base_addr +
			(sizeof(struct f1a_0d_control_3_4)*f1a->button_count);
	...
	if (button_query->has_perbutton_threshold &&
		...
		error = rmi_read_block(rmi_dev, ctrl_base_addr,
			...
			f1a->button_count);
		if (error < 0) {
			printk( "FTUCH : F1A Device(%s) failed to read f1a_0d_control_5(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
		ctrl_base_addr = ctrl_base_addr +
	...
	if (button_query->has_release_threshold) {
		...
		error = rmi_read_block(rmi_dev, ctrl_base_addr,
				...
				sizeof(u8));
		if (error < 0) {
			printk( "FTUCH : F1A Device(%s) failed to read f1a_0d_control_6(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	...
	if (button_query->has_strongestbtn_hysteresis) {
		...
		error = rmi_read_block(rmi_dev, ctrl_base_addr,
				...
				sizeof(u8));
		if (error < 0) {
			printk( "FTUCH : F1A Device(%s) failed to read f1a_0d_control_7(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
		ctrl_base_addr = ctrl_base_addr + sizeof(u8);
	}
	...
	if (button_query->has_filter_strength) {
		...
		error = rmi_read_block(rmi_dev, ctrl_base_addr,
				...
				sizeof(u8));
		if (error < 0) {
			printk( "FTUCH : F1A Device(%s) failed to read f1a_0d_control_8(%d)\n", dev_name( &rmi_dev->dev ), error );
			return error;
		}
	}
	return 0;
...
static int rmi_f1a_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f1a) {
		printk( "FTUCH : F1A Device(%s) failed to allocate function data\n", dev_name( &fc->dev ) );
		return -ENOMEM;
	}
	fc->data = f1a;
	...
	if (rc < 0) {
		printk( "FTUCH : F1A Device(%s) failed to read query register\n", dev_name( &fc->dev ) );
		return rc;
	}
	...
	if (!f1a->button_data_buffer) {
		printk( "FTUCH : F1A Device(%s) failed to allocate button data buffer\n", dev_name( &fc->dev ) );
		return -ENOMEM;
	}
	...
	if (!f1a->button_map) {
		printk( "FTUCH : F1A Device(%s) failed to allocate button map\n", dev_name( &fc->dev ) );
		return -ENOMEM;
	}
	if (f1a->button_query.has_interrupt_enable) {
		...
		if (!f1a->button_control.button_int_enable) {
			printk( "FTUCH : F1A Device(%s) failed to allocate interrupt button\n", dev_name( &fc->dev ) );
			return -ENOMEM;
		}
	}
	if (f1a->button_query.has_multibutton_select) {
		...
		if (!f1a->button_control.multi_button) {
			printk( "FTUCH : F1A Device(%s) failed to allocate multi button group select\n", dev_name( &fc->dev ) );
			return -ENOMEM;
		}
	}
	if (f1a->button_query.has_tx_rx_map) {
		...
		if (!f1a->button_control.electrode_map) {
			printk( "FTUCH : F1A Device(%s) failed to allocate f1a_0d_control_3_4\n", dev_name( &fc->dev ) );
			return -ENOMEM;
		}
	}
	if (f1a->button_query.has_perbutton_threshold) {
		...
		if (!f1a->button_control.button_threshold) {
			printk( "FTUCH : F1A Device(%s) failed to allocate button threshold\n", dev_name( &fc->dev ) );
			return -ENOMEM;
		}
	}
	return 0;
...
static int rmi_f1a_initialize(struct rmi_function_container *fc)
{
	...
	printk( "FTUCH : F1A Device(%s) intializing values\n", dev_name( &fc->dev ) );

	/* initial all default values for f1a data here */
	pdata = to_rmi_platform_data(rmi_dev);
	if (pdata) {
		if (!pdata->f1a_button_map)
			printk( "FTUCH : F1A Device(%s) button_map is NULL\n", dev_name( &fc->dev ) );
		else if (pdata->f1a_button_map->nbuttons !=
				f1a->button_count)
			printk( "FTUCH : F1A Device(%s) platformdata button map size (%d) != number of buttons on device (%d) - ignored\n", dev_name( &fc->dev ), pdata->f1a_button_map->nbuttons, f1a->button_count );
		else if (!pdata->f1a_button_map->map)
			printk( "FTUCH : F1A Device(%s) platformdata button map is missing\n", dev_name( &fc->dev ) );
		else
	...
	if (rc < 0) {
		printk( "FTUCH : F1A Device(%s) failed to initialize F1a control params\n", dev_name( &fc->dev ) );
		return rc;
	}
...
static int rmi_f1a_register_device(struct rmi_function_container *fc)
{
	...
	if (!input_dev) {
		printk( "FTUCH : F1A Device(%s) failed to allocate input device\n", dev_name( &fc->dev ) );
		return -ENOMEM;
	}
	...
	if (rc < 0) {
		printk( "FTUCH : F1A Device(%s) failed to register input device\n", dev_name( &fc->dev ) );
...
static int rmi_f1a_create_sysfs(struct rmi_function_container *fc)
{
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (sysfs_create_file
			...
			printk( "FTUCH : F1A Device(%s) failed to create sysfs file for %s\n", dev_name( &fc->dev ), attrs[attr_count].attr.name );
			rc = -ENODEV;
...
static int rmi_f1a_config(struct rmi_function_container *fc)
{
	...
	if (button_query->has_general_control) {
		retval = rmi_write_block(fc->rmi_dev,
				...
				sizeof(union f1a_0d_control_0));
		if (retval < 0) {
			printk( "FTUCH : F1A Device(%s) could not write general_control to 0x%x\n", dev_name( &fc->dev ), fc->fd.control_base_addr );
			return retval;
		}
	}
	if (button_query->has_interrupt_enable) {
		...
		if (retval < 0) {
			printk( "FTUCH : F1A Device(%s) could not write interrupt_enable_store to 0x%x\n", dev_name( &fc->dev ), data->button_int_enable_address );
			return retval;
		}
	}
	if (button_query->has_multibutton_select) {
		retval = rmi_write_block(fc->rmi_dev,
				...
				data->button_bitmask_size);
		if (retval < 0) {
			printk( "FTUCH : F1A Device(%s) could not write multi_button_store to 0x%x\n", dev_name( &fc->dev ), data->multi_button_address );
			return -EINVAL;
		}
	}
	if (button_query->has_tx_rx_map) {
		retval = rmi_write_block(fc->rmi_dev,
				...
				data->button_count);
		if (retval < 0) {
			printk( "FTUCH : F1A Device(%s) could not electrode_map_store to 0x%x\n", dev_name( &fc->dev ), data->electrode_map_address );
			return -EINVAL;
		}
	}
	if (button_query->has_perbutton_threshold) {
		retval = rmi_write_block(fc->rmi_dev,
				...
				data->button_count);
		if (retval < 0) {
			printk( "FTUCH : F1A Device(%s) could not button_threshold_store to 0x%x\n", dev_name( &fc->dev ), data->button_threshold_address );
			return retval;
		}
	}
	if (button_query->has_release_threshold) {
		retval = rmi_write_block(fc->rmi_dev,
			...
			sizeof(u8));
		if (retval < 0) {
			printk( "FTUCH : F1A Device(%s) could not write  button_release_threshold store to 0x%x\n", dev_name( &fc->dev ), data->button_release_threshold_address );
			return -EINVAL;
		}
	}
	if (button_query->has_strongestbtn_hysteresis) {
		retval = rmi_write_block(fc->rmi_dev,
			...
			sizeof(u8));
		if (retval < 0) {
			printk( "FTUCH : F1A Device(%s) could not write strongest_button_hysteresis store to 0x%x\n", dev_name( &fc->dev ), data->strongest_button_hysteresis_address );
			return -EINVAL;
		}
	}
	if (button_query->has_filter_strength) {
		retval = rmi_write_block(fc->rmi_dev,
				...
				sizeof(u8));
		if (retval < 0) {
			printk( "FTUCH : F1A Device(%s) could not write filter_strength store to 0x%x\n", dev_name( &fc->dev ), data->filter_strength_address );
			return -EINVAL;
		}
	}
	return 0;
...
int rmi_f1a_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	if (error < 0) {
		printk( "FTUCH : F1A Device(%s) failed to read button data registers\n", dev_name( &fc->dev ) );
		return error;
	}
...
static int __init rmi_f1a_module_init(void)
{
	...
	if (error < 0) {
		printk( "FTUCH : F1A register failed!!!\n" );
		return error;
	}
					 ...
					 const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &new_value) < 1) {
		printk( "FTUCH : F1A Device(%s) ERROR - filter_mode_store has an invalid len\n", dev_name( dev ) );
		return -EINVAL;
	}

	if (new_value < FILTER_MODE_MIN || new_value > FILTER_MODE_MAX) {
		printk( "FTUCH : F1A Device(%s) ERROR - filter_mode_store has an invalid value %d\n", dev_name( dev ), new_value );
		return -EINVAL;
	}
	...
	if (result < 0) {
		...
		printk( "FTUCH : F1A Device(%s) could not write filter_mode_store to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
					 ...
					 const char *buf, size_t count)
{
	...
	data = fc->data;
	if (sscanf(buf, "%u", &new_value) < 1) {
		printk( "FTUCH : F1A Device(%s) ERROR - multibutton_report_store has an invalid len\n", dev_name( dev ) );
		return -EINVAL;
	}

	if (new_value < MULTI_BUTTON_REPORT_MIN ||
		new_value > MULTI_BUTTON_REPORT_MAX) {
		printk( "FTUCH : F1A Device(%s) ERROR - multibutton_report_store has an invalid value %d\n", dev_name( dev ), new_value );
		return -EINVAL;
	}
	...
	if (result < 0) {
		data->button_control.general_control.multibutton_report =
				old_value;
		printk( "FTUCH : F1A Device(%s) could not write multibutton_report_store to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		if (len > 0) {
			...
			printk( "FTUCH : F1A Device(%s) failed to build interrupt button buffer(%d)\n", dev_name( dev ), len );
	...
	if (len > 0)
		...
		printk( "FTUCH : F1A Device(%s) failed to append carriage return\n", dev_name( dev ) );
	return total_len;
					 ...
					 const char *buf, size_t count)
{
	...
	if (!new_value) {
		printk( "FTUCH : F1A Device(%s) ERROR - failed to allocate button interrupt enable\n", dev_name( dev ) );
		return -ENOMEM;
	}
	for (i = 0; i < data->button_count && *buf != 0;
		...
		if ((result != 1) || (button != 0 && button != 1)) {
			printk( "FTUCH : F1A Device(%s) ERROR - interrupt enable button for button %d is not a valid value 0x%x\n", dev_name( dev ), i, button );
			kfree(new_value);
			return -EINVAL;
		}
		if (button)
	...
	if (button_count != data->button_count) {
		printk( "FTUCH : F1A Device(%s) ERROR - interrupt enable button count of %d doesn't match device button count of %d\n", dev_name( dev ), button_count, data->button_count );
		kfree(new_value);
		return -EINVAL;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F1A Device(%s) could not write interrupt_enable_store to 0x%x\n", dev_name( dev ), data->button_int_enable_address );
		return retval;
	}
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		if (len > 0) {
			...
			printk( "FTUCH : F1A Device(%s) failed to build multibutton buffer(%d)\n", dev_name( dev ), len );
			return snprintf(buf, PAGE_SIZE, "unknown\n");
		}
	}
	len = snprintf(current_buf, PAGE_SIZE - total_len, "\n");
					 ...
					 const char *buf, size_t count)
{
	...
	new_value = kzalloc(data->button_bitmask_size, GFP_KERNEL);
	if (!new_value) {
		printk( "FTUCH : F1A Device(%s) ERROR - failed to allocate multi button\n", dev_name( dev ) );
		return -ENOMEM;
	}
	for (i = 0; i < data->button_count && *buf != 0;
	     i++, buf += 2) {
		...
		if ((result < 1) || (button < 0 || button > 1)) {
			printk( "FTUCH : F1A Device(%s) ERROR - multibutton for button %d is not a valid value 0x%x\n", dev_name( dev ), i, button );
			kfree(new_value);
			return -EINVAL;
		}
	...
	/* Make sure the button count matches */
	if (button_count != data->button_count) {
		printk( "FTUCH : F1A Device(%s) ERROR - multibutton count of %d doesn't match device button count of %d\n", dev_name( dev ), button_count, data->button_count );
		kfree(new_value);
		return -EINVAL;
	}
	/* write back to the control register */
	retval = rmi_write_block(fc->rmi_dev, data->multi_button_address,
			...
			data->button_bitmask_size);
	if (retval < 0) {
		printk( "FTUCH : F1A Device(%s) could not write multibutton_store to 0x%x\n", dev_name( dev ), data->multi_button_address );
		kfree(new_value);
		return -EINVAL;
	}
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		if (len > 0) {
			...
			printk( "FTUCH : F1A Device(%s) failed to build electrode map buffer(%d)\n", dev_name( dev ), len );
			return snprintf(buf, PAGE_SIZE, "unknown\n");
		}
	}
	len = snprintf(current_buf, PAGE_SIZE - total_len, "\n");
					 ...
					 const char *buf, size_t count)
{
	...
	new_value = kzalloc(data->button_count *
			sizeof(struct f1a_0d_control_3_4), GFP_KERNEL);
	if (!new_value) {
		printk( "FTUCH : F1A Device(%s) ERROR - failed to allocate eletrode map\n", dev_name( dev ) );
		return -ENOMEM;
	}
	for (i = 0; i < data->button_count && *buf != 0; i++) {
		...
		if (result < 1) {
			printk( "FTUCH : F1A Device(%s) ERROR - scanf threshold button\n", dev_name( dev ) );
			kfree(new_value);
			return -EINVAL;
		}
		...
		if ((result < 3) || (colon != ':') ||
			...
			printk( "FTUCH : F1A Device(%s) ERROR - electrode map for button %d is not a valid value 0x%x:0x%x\n", dev_name( dev ), i, new_value[i].transmitterbutton, new_value[i].receiverbutton );
			kfree(new_value);
			return -EINVAL;
		}
		button_count++;
	...
	if (button_count != data->button_count) {
		printk( "FTUCH : F1A Device(%s) ERROR - button map count of %d doesn't match device button count of %d\n", dev_name( dev ), button_count, data->button_count );
		kfree(new_value);
		return -EINVAL;
	}
	/* write back to the control register */
	...
	if (retval < 0) {
		printk( "FTUCH : F1A Device(%s) could not transmitter_btn_store to 0x%x\n", dev_name( dev ), data->electrode_map_address );
		return -EINVAL;
		kfree(new_value);
	}
	for (i = 0; i < data->button_count; i++) {
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		if (len > 0) {
			...
			printk( "FTUCH : F1A Device(%s) failed to build threshold_button(%d)\n", dev_name( dev ), len );
			return snprintf(buf, PAGE_SIZE, "unknown\n");
		}
	}
	len = snprintf(current_buf, PAGE_SIZE - total_len, "\n");
					 ...
					 const char *buf, size_t count)
{
	...
	if (!new_value) {
		printk( "FTUCH : F1A Device(%s) ERROR - failed to allocate threshold button\n", dev_name( dev ) );
		return -ENOMEM;
	}
	...
	for (i = 0; i < data->button_count && *buf != 0; i++) {
		...
		if ((result < 1) ||
			...
			printk( "FTUCH : F1A Device(%s) ERROR - threshold_button for button %d is not a valid value 0x%x\n", dev_name( dev ), i, new_value[i] );
			kfree(new_value);
			return -EINVAL;
		}
		button_count++;
	...
	if (retval < 0) {
		printk( "FTUCH : F1A Device(%s) could not threshold_button_store to 0x%x\n", dev_name( dev ), data->button_threshold_address );
		kfree(new_value);
		return -EINVAL;
	}
					 ...
					 const char *buf, size_t count)
{
	...
	if ((len < 1) ||
		...
		printk( "FTUCH : F1A Device(%s) ERROR - release threshold_button is not a valid value 0x%x\n", dev_name( dev ), new_value );
		return -EINVAL;
	}
	...
	if (len < 0) {
		...
		printk( "FTUCH : F1A Device(%s) could not button_release_threshold_store to 0x%x\n", dev_name( dev ), data->button_release_threshold_address );
		return len;
	}
					 ...
					 const char *buf, size_t count)
{
	...
	if ((len < 1) ||
		...
		printk( "FTUCH : F1A Device(%s) ERROR - strongest button hysteresis is not a valid value 0x%x\n", dev_name( dev ), new_value );
		return -EINVAL;
	}
	...
	if (len < 0) {
		...
		printk( "FTUCH : F1A Device(%s) could not strongest_button_hysteresis_store to 0x%x\n", dev_name( dev ), data->strongest_button_hysteresis_address );
		return len;
	}
					 ...
					 const char *buf, size_t count)
{
	...
	if ((len < 1) ||
		...
		printk( "FTUCH : F1A Device(%s) ERROR - filter strength button is not a valid value 0x%x\n", dev_name( dev ), new_value );
		return -EINVAL;
	}
	...
	if (len < 0) {
		...
		printk( "FTUCH : F1A Device(%s) could not filter_strength_store to 0x%x\n", dev_name( dev ), data->filter_strength_address );
		return len;
	}
				...
				char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		if (len > 0) {
			...
			printk( "FTUCH : F1A Device(%s) failed to build button map buffer(%d)\n", dev_name( dev ), len );
			return snprintf(buf, PAGE_SIZE, "unknown\n");
		}
	}
	len = snprintf(current_buf, PAGE_SIZE - total_len, "\n");
				...
				size_t count)
{
	...
	for (i = 0; i < data->button_count && *buf != 0;
		...
		/* Make sure the key is a valid key */
		if (button > KEY_MAX) {
			printk( "FTUCH : F1A Device(%s) ERROR - button map for button %d is not a valid value 0x%x\n", dev_name( dev ), i, button );
			retval = -EINVAL;
			goto err_ret;
		}
