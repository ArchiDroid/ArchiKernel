...
static int rmi_f34_init(struct rmi_function_container *fc)
{
	...
	printk( "FTUCH : F34 Device(%s) intializing values\n", dev_name( &fc->dev ) );
...
static int rmi_f34_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f34) {
		printk( "FTUCH : F34 Device(%s) failed to allocate rmi_fn_34_data\n", dev_name( &fc->dev ) );
		return -ENOMEM;
	}
	fc->data = f34;
...
static int rmi_f34_initialize(struct rmi_function_container *fc)
{
	...
	if (retval < 0) {
		printk( "FTUCH : F34 Device(%s) could not read bootloaderid from 0x%04x\n", dev_name( &fc->dev ), query_base_addr );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F34 Device(%s) could not read block size from 0x%04x(%d)\n", dev_name( &fc->dev ), query_base_addr + BLK_SZ_OFF, retval );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F34 Device(%s) couldn't read image block count from 0x%x(%d)\n", dev_name( &fc->dev ), query_base_addr + IMG_BLK_CNT_OFF, retval );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F34 Device(%s) couldn't read config block count from 0x%x(%d)\n", dev_name( &fc->dev ), query_base_addr + CFG_BLK_CNT_OFF, retval );
		return retval;
	}
...
static int rmi_f34_create_sysfs(struct rmi_function_container *fc)
{
	...
	rc = sysfs_create_bin_file(&fc->dev.kobj,
				&dev_attr_data);
	if (rc < 0) {
		printk( "FTUCH : F34 Device(%s) failed to create sysfs file for F34 data(%d)\n", dev_name( &fc->dev ), rc );
		return -ENODEV;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		if (sysfs_create_file
			...
			printk( "FTUCH : F34 Device(%s) failed to create sysfs file for %s\n", dev_name( &fc->dev ), attrs[attr_count].attr.name );
			rc = -ENODEV;
...
static int f34_read_status(struct rmi_function_container *fc)
{
	...
	if (retval < 0) {
		printk( "FTUCH : F34 Device(%s) could not read status from 0x%x\n", dev_name( &fc->dev ), data_base_addr + instance_data->blocksize + BLK_NUM_OFF );
		status = 0xff;	/* failure */
	}
					...
					size_t count)
{
	...
	if (error < 0) {
		printk( "FTUCH : F34 Device(%s) could not write bootloader id to 0x%x\n", dev_name( dev ), data_base_addr + BLK_NUM_OFF );
		return error;
	}
				...
				size_t count)
{
	...
	if ((val != ENABLE_FLASH_PROG) && !instance_data->inflashprogmode) {
		printk( "FTUCH : F34 Device(%s) CANNOT SEND CMD %d TO SENSOR - NOT IN FLASH PROG MODE\n", dev_name( dev ), data_base_addr );
		return -EINVAL;
	}
	...
	case WRITE_CONFIG_BLOCK:
		...
		if (error < 0) {
			printk( "FTUCH : F34 Device(%s) could not write command 0x%02x to 0x%04x\n", dev_name( dev ), instance_data->cmd, data_base_addr + instance_data->blocksize + BLK_NUM_OFF );
			return error;
		}
					...
					size_t count)
{
	...
	if (error < 0) {
		printk( "FTUCH : F34 Device(%s) could not write block number %u to 0x%x\n", dev_name( dev ), instance_data->blocknum, data_base_addr );
		return error;
	}
				     ...
				     const char *buf, size_t count)
{
	...
	if (instance_data->cmd != ENABLE_FLASH_PROG) {
		printk( "FTUCH : F34 Device(%s) NOT IN FLASH PROG MODE - CAN'T RESCAN PDT\n", dev_name( dev ) );
		return -EINVAL;
	}
	...
	if (rescan) {
		...
		for (i = PDT_START_SCAN_LOCATION; i >= PDT_END_SCAN_LOCATION;
			...
			if (retval != sizeof(pdt_entry)) {
				printk( "FTUCH : F34 Device(%s) err frm rmi_read_block pdt entry data from PDT(%d)\n", dev_name( dev ), retval );
				return retval;
			}
		...
		if (!fn01found || !fn34found) {
			printk( "FTUCH : F34 Device(%s) failed to find fn$01 or fn$34 trying to do rescan PDT\n", dev_name( dev ) );
			return -EINVAL;
		}
	}
...
#endif
{
	...
	if (count != instance_data->blocksize) {
		printk( "FTUCH : F34 Device(%s) incorrect F34 block size %d. Expected size %d\n", dev_name( dev ), count, instance_data->blocksize );
		return -EINVAL;
	}
	...
	if (error < 0) {
		printk( "FTUCH : F34 Device(%s) could not read data from 0x%04x\n", dev_name( dev ), data_base_addr + BLK_NUM_OFF );
		return error;
	}
...
#endif
{
	...
	if (count) {
		...
		if (error < 0) {
			printk( "FTUCH : F34 Device(%s) could not write block data to 0x%x\n", dev_name( dev ), data_base_addr + BLK_NUM_OFF );
			return error;
		}
	}
...
static int __init rmi_f34_module_init(void)
{
	...
	if (error < 0) {
		printk( "FTUCH : F34 register failed!!!\n" );
		return error;
	}
