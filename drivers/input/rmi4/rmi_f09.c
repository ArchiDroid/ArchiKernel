...
static int rmi_f09_init(struct rmi_function_container *fc)
{
	...
	printk( "FTUCH : F09 Device(%s) intializing values\n", dev_name( &fc->dev ) );
...
static int rmi_f09_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f09) {
		printk( "FTUCH : F09 Device(%s) failed to allocate rmi_fn_09_data\n", dev_name( &fc->dev ) );
		return -ENOMEM;
	}
	fc->data = f09;
...
static int rmi_f09_initialize(struct rmi_function_container *fc)
{
	...
	if (rc < 0) {
		printk( "FTUCH : F09 Device(%s) failed to read query register from 0x%04x\n", dev_name( &fc->dev ), query_base_addr );
		return rc;
	}
...
static int rmi_f09_create_sysfs(struct rmi_function_container *fc)
{
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		if (sysfs_create_file
			...
			printk( "FTUCH : F09 Device(%s) failed to create sysfs file for %s\n", dev_name( &fc->dev ), attrs[attr_count].attr.name );
			rc = -ENODEV;
...
static int rmi_f09_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	if (error < 0) {
		printk( "FTUCH : F09 Device(%s) failed to read command register\n", dev_name( &fc->dev ) );
		return error;
	}
	/* If the command register is cleared, meaning the value is 0 */
	if (data->status == ECONNRESET) {
		printk( "FTUCH : F09 Device(%s) RESET occured(%#04x)\n", dev_name( &rmi_dev->dev ), data->status );
	} else if (data->cmd.run_bist) {
		printk( "FTUCH : F09 Device(%s) command register not cleared(%#04x)\n", dev_name( &rmi_dev->dev ), data->cmd.run_bist );
	}
	return 0;
}
...
static int __init rmi_f09_module_init(void)
{
	...
	if (error < 0) {
		printk( "FTUCH : F09 register failed!!!\n" );
		return error;
	}
					  ...
					  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &new_value) != 1) {
		printk( "FTUCH : F09 Device(%s) ERROR - hostTestEnable has an invalid length\n", dev_name( dev ) );
		return -EINVAL;
	}

	if (new_value < 0 || new_value > 1) {
		printk( "FTUCH : F09 Device(%s) invalid hostTestEnable bit %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.query_base_addr,
		data->query.host_test_enable);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write hostTestEnable to 0x%x\n", dev_name( dev ), fc->fd.query_base_addr );
		return result;
	}
					...
					const char *buf, size_t count)
{
	...
	data = fc->data;
	if (sscanf(buf, "%u", &new_value) != 1) {
		printk( "FTUCH : F09 Device(%s) ERROR - run_bist_store has an invalid len\n", dev_name( dev ) );
		return -EINVAL;
	}

	if (new_value < 0 || new_value > 1) {
		printk( "FTUCH : F09 Device(%s) invalid run_bist bit %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.command_base_addr,
		data->cmd.run_bist);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write run_bist_store to 0x%x\n", dev_name( dev ), fc->fd.command_base_addr );
		return result;
	}
					...
					const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u %u %u", &new_low, &new_high, &new_diff) != 3) {
		printk( "FTUCH : F09 Device(%s) ERROR - f09_control_test1_store has an invalid len\n", dev_name( dev ) );
		return -EINVAL;
	}

	if (new_low < 0 || new_low > 1 || new_high < 0 || new_high
			|| new_diff < 0 || new_diff) {
		printk( "FTUCH : F09 Device(%s) invalid f09_control_test1_diff bit %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test1_limit_low);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write f09_control_test1_limit_low to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test1_limit_high);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write f09_control_test1_limit_high to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test1_limit_diff);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write f09_control_test1_limit_diff to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
					...
					const char *buf, size_t count)
{
	...
	if (new_low < 0 || new_low > 1 || new_high < 0 || new_high > 1 ||
		...
		printk( "FTUCH : F09 Device(%s) invalid f09_control_test2_diff bit %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test2_limit_low);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write f09_control_test2_limit_low to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test2_limit_high);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write f09_control_test2_limit_high to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test2_limit_diff);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write f09_control_test2_limit_diff to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
					...
					const char *buf, size_t count)
{
	...
	data = fc->data;
	if (sscanf(buf, "%u", &new_value) != 1) {
		printk( "FTUCH : F09 Device(%s) ERROR - test_number_control_store has an invalid len\n", dev_name( dev ) );
		return -EINVAL;
	}

	if (new_value < 0 || new_value > 1) {
		printk( "FTUCH : F09 Device(%s) invalid test_number_control bit %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->data.test_number_control);
	if (result < 0) {
		printk( "FTUCH : F09 Device(%s) could not write test_number_control_store to 0x%x\n", dev_name( dev ), fc->fd.data_base_addr );
		return result;
	}
