				     ...
				     const char *buf, size_t count)
{
	...
	if (reset) {
		...
		if (retval < 0) {
			printk( "FTUCH : F01 Device(%s) failed to issue reset command(%d)\n", dev_name( dev ), retval );
			return retval;
		}
	}
					 ...
					 size_t count)
{
	...
	if (retval < 0 || !RMI_IS_VALID_SLEEPMODE(new_value)) {
		printk( "FTUCH : F01 Device(%s) invalid sleep mode %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		printk( "FTUCH : F01 Device(%s) failed to write sleep mode(%d)\n", dev_name( dev ), retval );
	return retval;
}
				       ...
				       size_t count)
{
	...
	if (retval < 0 || new_value < 0 || new_value > 1) {
		printk( "FTUCH : F01 Device(%s) invalid nosleep bit %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		printk( "FTUCH : F01 Device(%s) failed to write nosleep bit\n", dev_name( dev ) );
	return retval;
}
				       ...
				       size_t count)
{
	...
	retval = strict_strtoul(buf, 10, &new_value);
	if (retval < 0 || new_value < 0 || new_value > 1) {
		printk( "FTUCH : F01 Device(%s) invalid chargerinput bit %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		printk( "FTUCH : F01 Device(%s) failed to write chargerinput bit\n", dev_name( dev ) );
	return retval;
}
				       ...
				       size_t count)
{
	...
	retval = strict_strtoul(buf, 10, &new_value);
	if (retval < 0 || new_value < 0 || new_value > 1) {
		printk( "FTUCH : F01 Device(%s) invalid reportrate bit %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		printk( "FTUCH : F01 Device(%s) failed to write reportrate bit\n", dev_name( dev ) );
	return retval;
}
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->irq_count; i++) {
		...
		if (len > 0) {
			...
			printk( "FTUCH : F01 Device(%s) failed to build interrupt_enable buffer(%d)\n", dev_name( dev ), len );
	...
	if (len > 0)
		...
		printk( "FTUCH : F01 Device(%s) failed to append carriage return\n", dev_name( dev ) );
	return total_len;
					  ...
					  const char *buf, size_t count)
{
	...
	for (i = 0; i < data->irq_count && *buf != 0;
		...
		if ((result != 1) ||
			...
			printk( "FTUCH : F01 Device(%s) ERROR - interrupt enable[%d] is not a valid value 0x%x\n", dev_name( dev ), i, interrupt_enable );
			return -EINVAL;
		}
	...
	if (irq_count != data->irq_count) {
		printk( "FTUCH : F01 Device(%s) ERROR - interrupt enable count of %d doesn't match device count of %d\n", dev_name( dev ), irq_count, data->irq_count );
		return -EINVAL;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) could not write interrupt_enable_store to 0x%x\n", dev_name( dev ), data->interrupt_enable_addr );
		return retval;
	}
					  ...
					  const char *buf, size_t count)
{
	...
	if (retval < 0 || new_value < 0 || new_value > 255) {
		printk( "FTUCH : F01 Device(%s) invalid doze interval %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	ctrl_base_addr = fc->fd.control_base_addr + sizeof(u8) +
			(sizeof(u8)*(data->num_of_irq_regs));
	printk( "FTUCH : F01 Device(%s) doze_interval store address %x, value %d\n", dev_name( dev ), ctrl_base_addr, data->device_control.doze_interval );
	...
	if (retval >= 0)
		...
		printk( "FTUCH : F01 Device(%s) failed to write doze interval\n", dev_name( dev ) );
	return retval;
					  ...
					  const char *buf, size_t count)
{
	...
	retval = strict_strtoul(buf, 10, &new_value);
	if (retval < 0 || new_value < 0 || new_value > 255) {
		printk( "FTUCH : F01 Device(%s) invalid wakeup threshold %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		printk( "FTUCH : F01 Device(%s) failed to write wakeup threshold\n", dev_name( dev ) );
	return retval;
					  ...
					  const char *buf, size_t count)
{
	...
	retval = strict_strtoul(buf, 10, &new_value);
	if (retval < 0 || new_value < 0 || new_value > 255) {
		printk( "FTUCH : F01 Device(%s) invalid doze holdoff %s\n", dev_name( dev ), buf );
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		printk( "FTUCH : F01 Device(%s) failed to write doze holdoff\n", dev_name( dev ) );
	return retval;
	...
	int num_of_irq_regs)
{
	...
	if (!f01) {
		printk( "FTUCH : F01 Device(%s) failed to allocate fn_01_data\n", dev_name( &fc->dev ) );
		return -ENOMEM;
	}
...
static int rmi_f01_initialize(struct rmi_function_container *fc)
{
	...
	retval = rmi_read_block(rmi_dev, fc->fd.control_base_addr,
			...
			sizeof(data->device_control.ctrl0.reg));
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to read F01 control\n", dev_name( &fc->dev ) );
		return retval;
	}
	...
	if (data->device_control.ctrl0.sleep_mode != RMI_SLEEP_MODE_NORMAL) {
		printk( "FTUCH : F01 Device(%s) non-zero sleep mode found. Clearing...\n", dev_name( &fc->dev ) );
	...
	retval = rmi_write_block(rmi_dev, fc->fd.control_base_addr,
			...
			sizeof(data->device_control.ctrl0.reg));
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to write F01 control\n", dev_name( &fc->dev ) );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to read F01 control interrupt enable register\n", dev_name( &fc->dev ) );
		goto error_exit;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to read Interrupt Status\n", dev_name( &fc->dev ) );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to read device query registers\n", dev_name( &fc->dev ) );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to read product ID\n", dev_name( &fc->dev ) );
		return retval;
	}
	data->product_id[RMI_PRODUCT_ID_LENGTH] = '\0';
	printk( "FTUCH : F01 Device(%s) found RMI device, manufacturer(%s), product(%s)\n", dev_name( &fc->dev ), data->basic_queries.manufacturer_id == 1 ? "synaptics" : "unknown", data->product_id );
	...
	if (data->basic_queries.has_adjustable_doze) {
		...
		if (pdata->power_management.doze_interval) {
			...
			retval = rmi_write(rmi_dev, data->doze_interval_addr,
					data->device_control.doze_interval);
			if (retval < 0) {
				printk( "FTUCH : F01 Device(%s) failed to configure F01 doze interval register\n", dev_name( &fc->dev ) );
				goto error_exit;
			}
		} else {
			retval = rmi_read(rmi_dev, data->doze_interval_addr,
					&data->device_control.doze_interval);
			if (retval < 0) {
				printk( "FTUCH : F01 Device(%s) failed to read F01 doze interval register\n", dev_name( &fc->dev ) );
				goto error_exit;
			}
		}
		...
		if (pdata->power_management.wakeup_threshold) {
			...
			retval = rmi_write(rmi_dev, data->wakeup_threshold_addr,
					data->device_control.wakeup_threshold);
			if (retval < 0) {
				printk( "FTUCH : F01 Device(%s) failed to configure F01 wakeup threshold register\n", dev_name( &fc->dev ) );
				goto error_exit;
			}
		} else {
			retval = rmi_read(rmi_dev, data->wakeup_threshold_addr,
					&data->device_control.wakeup_threshold);
			if (retval < 0) {
				printk( "FTUCH : F01 Device(%s) failed to read F01 wakeup threshold register\n", dev_name( &fc->dev ) );
				goto error_exit;
			}
		}
	}
	...
	if (data->basic_queries.has_adjustable_doze_holdoff) {
		...
		if (pdata->power_management.doze_holdoff) {
			...
			retval = rmi_write(rmi_dev, data->doze_holdoff_addr,
					data->device_control.doze_holdoff);
			if (retval < 0) {
				printk( "FTUCH : F01 Device(%s) failed to configure doze holdoff register\n", dev_name( &fc->dev ) );
				goto error_exit;
			}
		} else {
			retval = rmi_read(rmi_dev, data->doze_holdoff_addr,
					&data->device_control.doze_holdoff);
			if (retval < 0) {
				printk( "FTUCH : F01 Device(%s) failed to read F01 doze holdoff register\n", dev_name( &fc->dev ) );
				goto error_exit;
			}
		}
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to read device status\n", dev_name( &fc->dev ) );
		goto error_exit;
	}

	if (data->device_status.unconfigured) {
		printk( "FTUCH : F01 Device(%s) reset during configuration process(%#02x)\n", dev_name( &fc->dev ), data->device_status.status_code );
		retval = -EINVAL;
		goto error_exit;
	}
...
static int rmi_f01_create_sysfs(struct rmi_function_container *fc)
{
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(fn_01_attrs);
		...
		if (retval < 0) {
			printk( "FTUCH : F01 Device(%s) failed to create sysfs file for %s\n", dev_name( &fc->dev ), fn_01_attrs[attr_count].attr.name );
...
static int rmi_f01_config(struct rmi_function_container *fc)
{
	...
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to write device_control.reg\n", dev_name( &fc->dev ) );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to write interrupt enable\n", dev_name( &fc->dev ) );
		return retval;
	}
...
static int rmi_f01_resume(struct rmi_function_container *fc)
{
	...
	retval = rmi_write_block(rmi_dev,
			...
			sizeof(struct f01_device_control_0));
	if (retval < 0)
		printk( "FTUCH : F01 Device(%s) failed to restore normal operation(%d)\n", dev_name( &fc->dev ), retval );
	else {
...
static int rmi_f01_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	sizeof(data->device_status.reg));
	if (retval < 0) {
		printk( "FTUCH : F01 Device(%s) failed to read device status(%d)\n", dev_name( &fc->dev ), retval );
		return retval;
	}
	if (data->device_status.unconfigured) {
		printk( "FTUCH : F01 Device(%s) reset detected\n", dev_name( &fc->dev ) );
...
static int __init rmi_f01_module_init(void)
{
	...
	if (error < 0) {
		printk( "FTUCH : F01 register failed!!!\n" );
		return error;
	}
