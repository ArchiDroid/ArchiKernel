...
static int rmi_f54_init(struct rmi_function_container *fc)
{
	...
	printk( "FTUCH : F54 Device(%s) intializing\n", dev_name( &fc->dev ) );
...
static int rmi_f54_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f54) {
		printk( "FTUCH : F54 Device(%s) failed to allocate rmi_fn_54_data\n", dev_name( &fc->dev ) );
		return -ENOMEM;
	}
	fc->data = f54;
...
static int rmi_f54_initialize(struct rmi_function_container *fc)
{
	...
	if (retval < 0) {
		printk( "FTUCH : F54 Device(%s) could not read query registers from 0x%04x\n", dev_name( &fc->dev ), fc->fd.query_base_addr );
		return retval;
	}
...
static int rmi_f54_create_sysfs(struct rmi_function_container *fc)
{
	...
	if (rc < 0) {
		printk( "FTUCH : F54 Device(%s) failed to create sysfs file for F54 data(%d)\n", dev_name( &fc->dev ), rc );
		return -ENODEV;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		if (sysfs_create_file
			...
			printk( "FTUCH : F54 Device(%s) failed to create sysfs file for %s\n", dev_name( &fc->dev ), attrs[attr_count].attr.name );
			rc = -ENODEV;
...
static int rmi_f54_reset(struct rmi_function_container *fc)
{
	...
	if (driver->restore_irq_mask) {
		printk( "FTUCH : F54 Device(%s) restoring interupts\n", dev_name( &fc->dev ) );
		driver->restore_irq_mask(fc->rmi_dev);
	} else {
		printk( "FTUCH : F54 Device(%s) no way to restore interrupts\n", dev_name( &fc->dev ) );
	}
...
static void rmi_f54_remove(struct rmi_function_container *fc)
{
	...
	printk( "FTUCH : F54 Device(%s) removing\n", dev_name( &fc->dev ) );
...
int rmi_f54_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	if (data->report_size == 0) {
		printk( "FTUCH : F54 Device(%s) invalid report type set. This should never happen\n", dev_name( &fc->dev ) );
		error = -EINVAL;
		goto error_exit;
	}
	/*
	...
	if (data->bufsize < data->report_size) {
		...
		if (!data->report_data) {
			printk( "FTUCH : F54 Device(%s) failed to allocate report_data\n", dev_name( &fc->dev ) );
			error = -ENOMEM;
			data->bufsize = 0;
	...
	if (error < 0)
		printk( "FTUCH : F54 Device(%s) failed to write fifo to zero\n", dev_name( &fc->dev ) );
	else
		error = rmi_read_block(fc->rmi_dev,
			fc->fd.data_base_addr + RMI_F54_REPORT_DATA_OFFSET,
			data->report_data, data->report_size);
	if (error < 0)
		printk( "FTUCH : F54 Device(%s) data read failed(%d)\n", dev_name( &fc->dev ), error );
	...
	printk( "FTUCH : F54 report data (raw hex):\n" );
	for (l = 0; l < data->report_size; l += 2) {
		printk( "%03d: 0x%02x%02x\n", l/2, data->report_data[l+1], data->report_data[l] );
	}
#endif
#if HUMAN_READABLE
	...
	case F54_FULL_RAW_CAP:
	case F54_FULL_RAW_CAP_RX_COUPLING_COMP:
		printk( "FTUCH : F54 report data (Image):\n" );
		int i, j, k;
		char c[2];
		short s;
		k = 0;
		...
		break;
	default:
		printk( "FTUCH : F54 report type %d debug image not supported\n", data->report_type );
	}
#endif
	error = IDLE;
error_exit:
...
static void clear_status_worker(struct work_struct *work)
{
	...
	if (data->status == BUSY) {
		...
		result = rmi_read_block(fc->rmi_dev, fc->fd.command_base_addr,
								&command, 1);
		if (result < 0) {
			printk( "FTUCH : F54 Device(%s) could not read get_report register from 0x%04x\n", dev_name( &fc->dev ), fc->fd.command_base_addr );
			data->status = -ETIMEDOUT;
		} else {
			if (command & GET_REPORT) {
				printk( "FTUCH : F54 Device(%s) report type unsupported\n", dev_name( &fc->dev ) );
				data->status = -EINVAL;
			} else {
				data->status = -ETIMEDOUT;
			}
...
static ssize_t rmi_fn_54_report_type_store(struct device *dev,
	...
	if (!is_report_type_valid(val)) {
		printk( "FTUCH : F54 Device(%s) report type %d is invalid\n", dev_name( dev ), ( u8 )val );
		return -EINVAL;
	}
	...
	if (instance_data->status != BUSY) {
		...
		if (result < 0) {
			printk( "FTUCH : F54 Device(%s) could not write report type to 0x%x\n", dev_name( dev ), fc->fd.data_base_addr );
			return result;
		}
		return count;
	} else {
		printk( "FTUCH : F54 Device(%s) report type cannot be changed in the middle of command\n", dev_name( dev ) );
		mutex_unlock(&instance_data->status_mutex);
		return -EINVAL;
	}
}
...
static ssize_t rmi_fn_54_get_report_store(struct device *dev,
	...
	if (!is_report_type_valid(instance_data->report_type)) {
		printk( "FTUCH : F54 Device(%s) report type %d is invalid\n", dev_name( dev ), instance_data->report_type );
		return -EINVAL;
	}
	mutex_lock(&instance_data->status_mutex);
	if (instance_data->status != IDLE) {
		if (instance_data->status != BUSY) {
			printk( "FTUCH : F54 Device(%s) status is in an abnormal state(0x%x)\n", dev_name( dev ), instance_data->status );
		}
		mutex_unlock(&instance_data->status_mutex);
		return count;
	}
	...
	if (driver->store_irq_mask)
		...
		printk( "FTUCH : F54 Device(%s) no way to store interupts\n", dev_name( dev ) );
	...
	mutex_unlock(&instance_data->status_mutex);
	if (result < 0) {
		printk( "FTUCH : F54 Device(%s) could not write command to 0x%x\n", dev_name( dev ), fc->fd.command_base_addr );
		return result;
	}
#if F54_WATCHDOG
...
static ssize_t rmi_fn_54_no_auto_cal_store(struct device *dev,
	...
	if (result < 0) {
		printk( "FTUCH : F54 Device(%s) could not write control to 0x%x\n", dev_name( dev ), fc->fd.control_base_addr );
		return result;
	}
...
static ssize_t rmi_fn_54_fifoindex_show(struct device *dev,
	...
	if (retval < 0) {
		printk( "FTUCH : F54 Device(%s) could not read fifoindex from 0x%04x\n", dev_name( dev ), fc->fd.data_base_addr + RMI_F54_FIFO_OFFSET );
		return retval;
	}
					...
					size_t count)
{
	...
	if (error < 0) {
		printk( "FTUCH : F54 Device(%s) could not write fifoindex to 0x%x\n", dev_name( dev ), fc->fd.data_base_addr + RMI_F54_FIFO_OFFSET );
		return error;
	}
	return count;
}
				...
				char *buf, loff_t pos, size_t count)
{
	...
	if (count < instance_data->report_size) {
		printk( "FTUCH : F54 Device(%s) report size too large for buffer: %d. Need at least: %d for Report type(%d)\n", dev_name( dev ), count, instance_data->report_size, instance_data->report_type );
	...
	if (instance_data->report_data) {
		...
		printk( "FTUCH : F54 Device(%s) report_data does not exist\n", dev_name( dev ) );
		mutex_unlock(&instance_data->data_mutex);
		return -EINVAL;
	}
}
...
static int __init rmi_f54_module_init(void)
{
	...
	if (error < 0) {
		printk( "FTUCH : F54 register failed!!!\n" );
		return error;
	}
	return 0;
}
