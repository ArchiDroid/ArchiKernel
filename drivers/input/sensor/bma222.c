...
#define BMA222_SHAKING_DETECT_THRESHOLD (10)    /* threshold of shaking detection under 2G */
...
#endif
static atomic_t bma222_diag = ATOMIC_INIT(0);
static atomic_t bma222_cnt = ATOMIC_INIT(0);
...
static int bma222_set_offset_filt_x(struct i2c_client *client, unsigned char
		offsetfilt)
{
	int comres = 0;
	unsigned char data;
	...
	data =  offsetfilt;
	comres = bma222_smbus_write_byte(client, BMA222_OFFSET_FILT_X_REG,
						&data);
...
static int bma222_get_offset_filt_x(struct i2c_client *client, unsigned char
						*offsetfilt)
{
	int comres = 0;
	unsigned char data;

	comres = bma222_smbus_read_byte(client, BMA222_OFFSET_FILT_X_REG,
						&data);
	*offsetfilt = data;
...
static int bma222_set_offset_filt_y(struct i2c_client *client, unsigned char
						offsetfilt)
{
	int comres = 0;
	unsigned char data;
	...
	data =  offsetfilt;
	comres = bma222_smbus_write_byte(client, BMA222_OFFSET_FILT_Y_REG,
						&data);

	return comres;
}

static int bma222_get_offset_filt_y(struct i2c_client *client, unsigned char
						*offsetfilt)
{
	int comres = 0;
	unsigned char data;

	comres = bma222_smbus_read_byte(client, BMA222_OFFSET_FILT_Y_REG,
						&data);
	*offsetfilt = data;
...
static int bma222_set_offset_filt_z(struct i2c_client *client, unsigned char
						offsetfilt)
{
	int comres = 0;
	unsigned char data;
	...
	data =  offsetfilt;
	comres = bma222_smbus_write_byte(client, BMA222_OFFSET_FILT_Z_REG,
						&data);

	return comres;
}

static int bma222_get_offset_filt_z(struct i2c_client *client, unsigned char
						*offsetfilt)
{
	int comres = 0;
	unsigned char data;

	comres = bma222_smbus_read_byte(client, BMA222_OFFSET_FILT_Z_REG,
						&data);
	*offsetfilt = data;
...
/* LGE_CHANGE_S [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
	...
	if(atomic_read(&bma222->fast_calib_rslt) != 0)
	{
		atomic_set(&bma222->fast_calib_rslt, 0);
		printk(KERN_INFO "[set] bma222->fast_calib_rslt:%d\n",atomic_read(&bma222->fast_calib_rslt));
	}
/* LGE_CHANGE_E [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
...
static int bma222_set_ee_w(struct i2c_client *client, unsigned char eew)
{
	int comres = 0;
	unsigned char data;

	comres = bma222_smbus_read_byte(client,
			BMA222_UNLOCK_EE_WRITE_SETTING__REG, &data);
...
static int bma222_set_ee_prog_trig(struct i2c_client *client)
{
	int comres = 0;
	unsigned char data;
	...
	comres = bma222_smbus_read_byte(client,
			BMA222_START_EE_WRITE_SETTING__REG, &data);
	data = BMA222_SET_BITSLICE(data,
				BMA222_START_EE_WRITE_SETTING, eeprog);
	comres = bma222_smbus_write_byte(client,
			BMA222_START_EE_WRITE_SETTING__REG, &data);
	return comres;
}

static int bma222_get_eeprom_writing_status(struct i2c_client *client,
...
static ssize_t bma222_eeprom_writing_store(struct device *dev,
		...
		const char *buf, size_t count)
{
	...
	/* unlock eeprom */
	if (bma222_set_ee_w(bma222->bma222_client, 1) < 0)
		return -EINVAL;

	printk(KERN_INFO "unlock eeprom successful\n");

	if (bma222_set_ee_prog_trig(bma222->bma222_client) < 0)
		return -EINVAL;
	printk(KERN_INFO "start update eeprom\n");

	do {
		mdelay(2);
		bma222_get_eeprom_writing_status(bma222->bma222_client, &tmp);
	...
	/* unlock eeprom */
	if (bma222_set_ee_w(bma222->bma222_client, 0) < 0)
		return -EINVAL;

	printk(KERN_INFO "lock eeprom successful\n");
/* LGE_CHANGE_S [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
	atomic_set(&bma222->fast_calib_rslt, 1);
	printk(KERN_INFO "[set] bma222->fast_calib_rslt:%d\n", atomic_read(&bma222->fast_calib_rslt));
/* LGE_CHANGE_E [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
	return count;
}
...
static int bma222_read_accel_x(struct i2c_client *client, short *a_x)
{
	...
	if (atomic_read(&bma222_diag) == 1)
		 atomic_inc(&bma222_cnt);
	comres = bma222_smbus_read_byte_block(client, BMA222_ACC_X8_LSB__REG,
			data, 2);
...
/* LGE_CHANGE_S [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
static int bma222_soft_reset(struct i2c_client *client)
{
        int comres = 0;
        unsigned char data = BMA222_EN_SOFT_RESET_VALUE ;

        comres = bma222_smbus_write_byte(client, BMA222_EN_SOFT_RESET__REG,
                                        &data);
	printk(KERN_INFO "===============bma222_soft_reset===============\n");
        return comres;
}
...
/* LGE_CHANGE_S [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
	struct bma222acc acc_cal;
	struct bma222acc acc_cal_pre;
/* LGE_CHANGE_E [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
	error = strict_strtoul(buf, 10, &data);
	if (error)
		return error;
/* LGE_CHANGE_S [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
	bma222_read_accel_xyz(bma222->bma222_client, &acc_cal_pre);
/* LGE_CHANGE_E [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
...
/* LGE_CHANGE_S [jiyeon.park@lge.com] 2012-02-09 sensor SURV shake detection*/
		bma222_read_accel_xyz(bma222->bma222_client, &acc_cal);
		if( (tmp == 0)  &&
		        ((abs(acc_cal.x - acc_cal_pre.x) > BMA222_SHAKING_DETECT_THRESHOLD)
		                || (abs((acc_cal.y - acc_cal_pre.y)) > BMA222_SHAKING_DETECT_THRESHOLD)
		                || (abs((acc_cal.z - acc_cal_pre.z)) > BMA222_SHAKING_DETECT_THRESHOLD))
		  )
		{                  
			printk(KERN_INFO "===============moved===============\n");
			bma222_soft_reset(bma222->bma222_client);
			return -EINVAL;
		}
		else
		{
...
static ssize_t bma222_offset_filt_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	struct i2c_client *client = to_i2c_client(dev);
	...
	if (bma222_get_offset_filt_x(bma222->bma222_client, &data) < 0)
		return sprintf(buf, "Read error\n");
...
static ssize_t bma222_offset_filt_x_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if (bma222_set_offset_filt_x(bma222->bma222_client, (unsigned
					char)data) < 0)
		return -EINVAL;
...
static ssize_t bma222_offset_filt_y_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	struct i2c_client *client = to_i2c_client(dev);
	...
	if (bma222_get_offset_filt_y(bma222->bma222_client, &data) < 0)
		return sprintf(buf, "Read error\n");
...
static ssize_t bma222_offset_filt_y_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if (bma222_set_offset_filt_y(bma222->bma222_client, (unsigned
					char)data) < 0)
		return -EINVAL;
...
static ssize_t bma222_offset_filt_z_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	struct i2c_client *client = to_i2c_client(dev);
	...
	if (bma222_get_offset_filt_z(bma222->bma222_client, &data) < 0)
		return sprintf(buf, "Read error\n");
...
static ssize_t bma222_offset_filt_z_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if (bma222_set_offset_filt_z(bma222->bma222_client, (unsigned
					char)data) < 0)
		return -EINVAL;
...
static ssize_t bma222_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short data;
	struct i2c_client *client = to_i2c_client(dev);
	struct bma222_data *bma222 = i2c_get_clientdata(client);
	if (bma222_read_accel_x(bma222->bma222_client, &data) < 0)
		return sprintf(buf, "Read error\n");
	
	return sprintf(buf, "%d\n", data);

}
static ssize_t bma222_y_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short data;
	struct i2c_client *client = to_i2c_client(dev);
	struct bma222_data *bma222 = i2c_get_clientdata(client);
	if (bma222_read_accel_y(bma222->bma222_client, &data) < 0)
		return sprintf(buf, "Read error\n");

	return sprintf(buf, "%d\n", data);

}
static ssize_t bma222_z_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	short data;
	struct i2c_client *client = to_i2c_client(dev);
	struct bma222_data *bma222 = i2c_get_clientdata(client);
	if (bma222_read_accel_z(bma222->bma222_client, &data) < 0)
		return sprintf(buf, "Read error\n");

	return sprintf(buf, "%d\n", data);

}
static ssize_t bma222_cal_result_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma222_data *bma222 = i2c_get_clientdata(client);
...
static ssize_t bma222_diag_cnt_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(&bma222_cnt));
}

static ssize_t bma222_diag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(&bma222_diag));
}
static ssize_t bma222_diag_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int value;
	...
	sscanf(buf, "%d", &value);
	if(value == 1){
		atomic_set(&bma222_diag,1);
	}else{
		atomic_set(&bma222_diag,0);
	}
	return count;
}
...
static DEVICE_ATTR(eeprom_writing, S_IRUGO|S_IWUSR|S_IWGRP,
		NULL, bma222_eeprom_writing_store);
static DEVICE_ATTR(offset_filt_x, S_IRUGO|S_IWUSR|S_IWGRP,
		bma222_offset_filt_x_show,
		bma222_offset_filt_x_store);
static DEVICE_ATTR(offset_filt_y, S_IRUGO|S_IWUSR|S_IWGRP,
		bma222_offset_filt_y_show,
		bma222_offset_filt_y_store);
static DEVICE_ATTR(offset_filt_z, S_IRUGO|S_IWUSR|S_IWGRP,
		bma222_offset_filt_z_show,
		bma222_offset_filt_z_store);
		...
		const struct i2c_device_id *id)
{
	...
	bma222_set_mode(client, BMA222_MODE_SUSPEND);
