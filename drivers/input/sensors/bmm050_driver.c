...
struct bmm_client_data {
	...
	struct bmm050_platform_data *platform_data;  /*MTD-PERIPHERAL-AC-layout-01*/
...
/*MTD-PERIPHERAL-AC-VREG_CONTROL-00+{*/
static int bmm050_power_control(struct bmm_client_data *bmm050,int onoff)
{
    int rc=0;
    if(IS_ERR(bmm050->vreg_l9))
    {
        PERR("Failed.");
        return -ENODEV;
    }
    ...
    {
        rc = regulator_set_optimum_mode(bmm050->vreg_l9, 10);
        if(rc < 0)
        {
            PERR( "Disable fail.");
            return rc;
...
static int bmm050_power_init(struct bmm_client_data *bmm050)
{
    int rc = 0;
    if(bmm050 == NULL)
    {
    ...
    bmm050->vreg_l9 = regulator_get(&bmm050->client->dev,"bmm_vreg");
    if(IS_ERR(bmm050->vreg_l9))
    {
        PERR("Can't power on init.");
        return -ENODEV;
    }
    rc = bmm050_power_control(bmm050,1);
    if(rc)
    {
        return rc;
    }
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE,"%d\n", client_data->platform_data->layout);
}
...
/*MTD-PERIPHERAL-AC-layout-01+{*/
static DEVICE_ATTR(layout, S_IRUGO|S_IWUSR,
...
static struct attribute *bmm_attributes[] = {
	...
	&dev_attr_delay.attr,
	&dev_attr_test.attr,
	&dev_attr_layout.attr,	/*MTD-PERIPHERAL-AC-layout-01*/
	NULL
};
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*MTD-PERIPHERAL-AC-layout-01+{*/
	client_data->platform_data = client->dev.platform_data;
	if(!client_data->platform_data)
	{
		PERR("Bosch Sensortec device can't find layout");
		err = -1;
		goto exit_err_clean;
	}
	if(fih_get_product_phase() == PHASE_PD)
	{
		client_data->platform_data->layout = 7;
	}
	/*MTD-PERIPHERAL-AC-layout-01+}*/
	client_data->client = client;

/*MTD-PERIPHERAL-AC-VREG_CONTROL-00+{*/
    err = bmm050_power_init(client_data);
	if(err < 0)
	{
		PERR("regulator get fail.");
	}
/*MTD-PERIPHERAL-AC-VREG_CONTROL-00+}*/
...
static void bmm_early_suspend(struct early_suspend *handler)
{
	...
	bmm050_power_control(client_data, 0);
	mutex_unlock(&client_data->mutex_power_mode);
...
static void bmm_late_resume(struct early_suspend *handler)
{
	...
	bmm050_power_control(client_data, 1);
